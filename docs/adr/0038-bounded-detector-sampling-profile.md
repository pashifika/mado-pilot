...
Detector quadrilaterals are clipped and mapped through the checked per-axis inverse scale into original source-view coordinates before candidate submission. Perspective crops for recognition are always read from the original mapped BGRA source using those source-coordinate quadrilaterals. Existing effective-region origin, `TransformSnapshot`, output-space conversion, and `FrameStamp` correlation then apply without caller scaling.
