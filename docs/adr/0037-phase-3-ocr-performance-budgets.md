...
The committed profile is included in benchmark-block and hard-budget drift registries. Its executable latency/resource constants are checked against the profile so a code/profile mismatch fails the repository suite. Final post-budget Apple execution and both hosted hard-gate jobs remain separate acceptance evidence; Windows target-specific final execution remains withheld with its precursor.
