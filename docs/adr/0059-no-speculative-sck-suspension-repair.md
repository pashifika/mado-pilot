...
A later exact-lifecycle campaign at source `ba6b0451a1d2b13a8fe418c477e91ea00fe9cff8` ran five single-display controls followed by five two-display mixed-DPI processes in one authenticated graphical session. Its schema recorded only display count and whether backing scales differed. All ten processes produced a fresh complete frame, kept only the expected detached result footprint while the old public session was gone, preserved mapping readability, and restored their exact process baselines. No process observed a non-complete status, refused callback, close/open overlap, target-readiness failure, or missing progress. The retained result is documented in [ScreenCaptureKit suspension signature capture](../evidence/macos-sck-suspension-signature-capture.md).
