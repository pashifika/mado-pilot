...
The Phase 4 change-detection topic adds a closed platform-neutral exact-RGBA
policy seam and deterministic offline `G-005` evaluator over repository-owned
recorded sequences. The local canonical comparison has zero exact-policy false
skips and rejects every lower-admitted-work candidate; ADR 0050 remains Proposed
until identical Windows and Apple Silicon hosted reports and independent review
pass. No watcher, query, scheduler, callback, facade, C, or C++ surface exists.
...
| Closed change-detection policy and recorded-sequence evaluator | Implemented on the Phase 4 topic in `mado-pilot-vision` and `mado-pilot-testkit`. Proposed ADR 0050 selects exact RGBA for compatible mapped regions, keeps analysis-always as fail-safe, and limits unchanged authority to routine-analysis admission. Both hosted report rows and independent review remain pending; no watcher or public facade operation exists |
...
| Change-detection policy and recorded-sequence contracts | The frozen `G-005` loader/evaluator tests cover strict manifests, exact fixture digests, ROI clipping, repeated pixels, geometry/epoch discontinuity, malformed/duplicate/reordered inputs, overflow, candidate failure, deterministic aggregate output, privacy, report/runtime descriptor parity, and proposed ADR 0050 drift. The closed runtime policy tests cover exact pixels, sequence gaps, fail-safe identity/geometry/ROI/format boundaries, unsupported policy codes, stable defaults, thread safety, and absence of worker state. Both hosted target report rows and independent review remain pending | Not applicable |
