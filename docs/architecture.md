...
| Open gates | [`G-013`](validation-gates.md#g-013) dual-4K production-capture acceptance and final-source Phase 1 reruns; [ADR 0026](adr/0026-windows-native-and-diagnostic-performance-budgets.md) accepts diagnostic timing and the original `native-phase2` workload sets, [ADR 0028](adr/0028-windows-window-message-performance-budgets.md) accepts ordinary `WindowMessage` timing/memory/pressure/cleanup ceilings, and [ADR 0031](adr/0031-windows-1280-production-capture-performance-budgets.md) accepts 1280×720 production capture/transitions | [`G-013`](validation-gates.md#g-013) final-source regression reruns; [ADR 0024](adr/0024-input-diagnostic-performance-budgets.md) accepts diagnostics, [ADR 0025](adr/0025-macos-native-input-performance-budgets.md) accepts native input/public-language costs, [ADR 0029](adr/0029-macos-process-directed-input.md) accepts the final controlled process-directed profiles and all fourteen topology-qualified owning-process pairs, and [ADR 0030](adr/0030-macos-production-capture-performance-budgets.md) accepts production capture and transition budgets |
...
| Windows native capture ownership policy | Implemented for the production Adapter's two-frame WGC pool, extent-derived process-shared retained maximum capped at 40, 128 MiB surface / 2 GiB session / 4 GiB process safety ceilings, lease-safe reuse, resize retirement, callback fence, and teardown. [ADR 0026](adr/0026-windows-native-and-diagnostic-performance-budgets.md) accepts the controlled `native-phase2` capture/transition profiles, and [ADR 0031](adr/0031-windows-1280-production-capture-performance-budgets.md) accepts the separate 1280×720 production capture/transition matrix and callback-copy/staging/resident budgets. The revision-bound dual-4K production profile remains open under `G-013` |
...
its callback-copy, staging, resident, transition, and cleanup budgets. The
dual-4K production row and final-source Phase 1 reruns remain open under
[`G-013`](validation-gates.md#g-013). The Adapter is reachable from the public
composition root through `mado_pilot::windows_engine`.
...
macOS production-capture matrix. The Windows dual-4K production row and
final-source regression reruns remain open under Phase 2
...
production-capture matrix, while the Windows dual-4K production row and
final-source reruns remain explicit `G-013` gaps.
