...
The registration repair accepts the callback's retained application, PID, and launch date as the new identity. Every later observation and termination still revalidates that identity against the current exact process lifetime. A deterministic scenario holds the workspace lookup in a transient missing state during the callback and proves launch succeeds before later exact cleanup.
...
Private fixture cleanup retains the exact application object, PID, and launch time. Containment checks that identity before at most one graceful and one forced termination request. If exact exit is still unconfirmed, a single serial queue performs observation only every 100 ms for at most twenty observations. Scheduled, active, completed, and exhausted counts are saturating private diagnostics; no released ABI field or feature-disabled symbol was added.
...
The linkage suite verifies that `mp_shim_fixture_cleanup_counts` is absent when `private-fixture` is disabled. Deterministic start scenarios cover multi-slice and no-deadline success, caller deadline, cancellation, normalized failure, submission and completion exceptions, simultaneous callers, and close/drop races. Fixture scenarios cover stale termination state, PID reuse, exact-probe failure, delayed exact death, observation exhaustion, overlapping releases, and launch callback registration lag.
