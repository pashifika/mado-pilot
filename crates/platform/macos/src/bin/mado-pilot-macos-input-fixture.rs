...
mod fixture {
    ...
    use crate::fixture_protocol::{
        ...
        WINDOW_POINTS, fixture_title, format_command_result_line, format_event_line,
        parse_command_line,
    };
    ...
    static EVENTS: OnceLock<Mutex<EventTotals>> = OnceLock::new();
    ...
    pub(super) fn run(
        ...
        RUN_NONCE
            ...
            .map_err(|_| Status(PLATFORM_FAILURE))?;
        EVENTS
            .set(Mutex::new(EventTotals::default()))
            .map_err(|_| Status(PLATFORM_FAILURE))?;
        PENDING_COMMAND
            .set(Mutex::new(None))
            .map_err(|_| Status(PLATFORM_FAILURE))?;
    ...
    unsafe extern "C" fn on_event(_context: *mut c_void, kind: u32, text_units: u32) {
        let _contained = std::panic::catch_unwind(|| {
            ...
            let mut events = events
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            if events.event_count() >= MAX_RECORDED_EVENTS as u64 {
                events.saturated = true;
                return;
            }
            let bounded_units = text_units.min(MAX_EVENT_TEXT_UNITS);
            events.saturated |= bounded_units != text_units;
            let count = match kind {
                crate::fixture_protocol::EVENT_POINTER_MOVE => &mut events.pointer_moves,
                crate::fixture_protocol::EVENT_POINTER_PRESS => &mut events.pointer_presses,
                crate::fixture_protocol::EVENT_POINTER_RELEASE => &mut events.pointer_releases,
                crate::fixture_protocol::EVENT_POINTER_SCROLL => &mut events.pointer_scrolls,
                crate::fixture_protocol::EVENT_KEY_DOWN => &mut events.key_downs,
                crate::fixture_protocol::EVENT_KEY_UP => &mut events.key_ups,
                crate::fixture_protocol::EVENT_FLAGS_CHANGED => &mut events.flags_changed,
                _ => {
                    events.saturated = true;
                    return;
                }
            ...
            *count += 1;
            events.text_units += u64::from(bounded_units);
            let reported = events.event_count();
            drop(events);
            write_protocol_line(format_args!(
                "{}",
                format_event_line(
    ...
    unsafe extern "C" {
        ...
        fn mp_fixture_control(version: u32, run_nonce: u64, nonce: u64, command: u32) -> u32;
        #[cfg(test)]
    ...
    #[cfg(test)]
    mod tests {
        use super::{UNSUPPORTED, mp_fixture_test_unsupported_renderer};
