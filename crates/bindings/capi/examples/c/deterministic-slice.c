 ...
 * the scene below is generated by the same integer arithmetic
 * `mado-pilot-testkit` uses: if the two ever drift apart, the template stops
 * being found where it is planted and this program fails.
 *
 ...
 *
 * A 96x64 pseudo-random background with a 12x10 patch planted at two offsets
 * and a half-contrast copy at a third. Every value comes from integer
 * arithmetic on the pixel coordinate, so both release targets build the same
 * bytes. See crates/support/testkit/src/match_fixtures.rs.
 * ----------------------------------------------------------------------- */
...
static const uint32_t PLANTED[2][2] = { { 20u, 12u }, { 60u, 40u } };
static const uint32_t DEGRADED[2] = { 20u, 44u };

static uint8_t byte_of(uint32_t value)
{
    return (uint8_t)(value & 0xffu);
}

static void patch_pixel(uint32_t x, uint32_t y, uint8_t out[3])
{
...
static void background_pixel(uint32_t x, uint32_t y, uint8_t out[3])
{
    uint32_t mixed = x * 2654435761u + y * 2246822519u;
    mixed ^= mixed >> 15;
    out[0] = byte_of(mixed);
    out[1] = byte_of(mixed >> 8);
    out[2] = byte_of(mixed >> 16);
}

static int covers(const uint32_t origin[2], uint32_t x, uint32_t y,
...
static void scene_pixel(uint32_t x, uint32_t y, uint8_t out[3])
{
    ...
    for (index = 0; index < 2; ++index) {
        if (covers(PLANTED[index], x, y, &px, &py)) {
            patch_pixel(px, py, out);
            return;
        }
    }

    if (covers(DEGRADED, x, y, &px, &py)) {
        uint8_t ideal[3];
        uint8_t noise[3];
        size_t channel;
        patch_pixel(px, py, ideal);
        background_pixel(x, y, noise);
        for (channel = 0; channel < 3; ++channel) {
            out[channel] = byte_of(((uint32_t)ideal[channel] + noise[channel]) / 2u);
        }
        return;
    }

    background_pixel(x, y, out);
}

static uint8_t* build_scene_rgba(size_t* out_len)
{
    size_t len = (size_t)SCENE_WIDTH * SCENE_HEIGHT * 4u;
...
int main(int argc, char** argv)
{
    ...
    scene = build_scene_rgba(&scene_len);
    if (scene == NULL) {
    ...
    frame_input.pixels.len = scene_len;
    ...
    /* 6. Map it. The mapped bytes stay readable after the session is gone. */
    {
        ...
        if (expect_ok(api->mapping_describe(mapping, &image), "mapping_describe")) {
            ...
            expect(image.bytes.len == (size_t)SCENE_WIDTH * SCENE_HEIGHT * 4u,
        ...
        for (at = 0; at < info.match_count; ++at) {
            ...
            if (expect_ok(api->result_match(found, at, &match), "result_match")) {
                ...
                printf(" at [%d, %d) x [%d, %d) score %.6f\n", match.bounds.left,
                       ...
                       match.score);
                expect(match.bounds.left == 20 || match.bounds.left == 60,
