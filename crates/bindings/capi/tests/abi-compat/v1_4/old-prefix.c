/*
 * The frozen ABI 1.4 C header, compiled against whatever library exists now.
 *
 ...
 * 720-byte declarations and executes the complete pre-1.3 common flow plus
 * every ABI 1.4 entry. The deterministic package has no OCR model, so the OCR
 ...
 *
 *   usage: madopilot-abi-compat-v1_4 --package <dir>
 */
...
static int run_the_flow(const madopilot_api_t* api, const char* package_dir)
{
...

    {
        ...
        madopilot_ocr_profile_options_t profile_options;
        madopilot_ocr_zone_scan_result_t* zone_result =
            (madopilot_ocr_zone_scan_result_t*)(void*)&failures;
        madopilot_ocr_zone_scan_result_info_t zone_info;
        ...
        expect(api->session_recognize(session, NULL, &operation, &ocr_result, &error) ==
               ...
               "ABI 1.4 session_recognize refusal");
        ...
        expect_ok(api->ocr_result_retain(NULL), "ABI 1.4 null OCR retain");
        expect_ok(api->ocr_result_release(NULL), "ABI 1.4 null OCR release");
        ...
        expect(api->engine_create_with_default_ocr(
                   ...
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 default OCR constructor refusal");
        expect(ocr_engine == NULL, "default OCR initializes its owner output");
        expect(error != NULL, "default OCR returns a typed refusal");
        api->error_release(error);
        error = NULL;

        memset(&profile_options, 0, sizeof(profile_options));
        profile_options.struct_size = (uint32_t)sizeof(profile_options);
        profile_options.kind = MADOPILOT_OCR_PROFILE_BOUNDED_DETECTOR;
        expect(api->engine_create_with_ocr_profile(
                   &source, NULL, &profile_options, &operation, &ocr_engine,
                   &error) == MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 profile constructor refusal");
        expect(ocr_engine == NULL,
               "profile constructor initializes its owner output");
        expect(error != NULL, "profile constructor returns a typed refusal");
        api->error_release(error);
        error = NULL;

        expect(api->session_scan_ocr_zones(
                   session, NULL, &operation, &zone_result, &error) ==
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 grouped OCR refusal");
        expect(zone_result == NULL,
               "grouped OCR initializes its owner output");
        expect(error != NULL, "grouped OCR returns a typed refusal");
        api->error_release(error);
        error = NULL;

        expect_ok(api->ocr_zone_scan_result_retain(NULL),
                  "ABI 1.4 null grouped OCR retain");
        expect_ok(api->ocr_zone_scan_result_release(NULL),
                  "ABI 1.4 null grouped OCR release");

        memset(&zone_info, 0xff, sizeof(zone_info));
        zone_info.struct_size = (uint32_t)sizeof(zone_info);
        expect(api->ocr_zone_scan_result_info(NULL, &zone_info) ==
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 null grouped OCR info");
        expect(zone_info.zone_count == 0,
               "grouped OCR info initializes its count");

        memset(&zone, 0xff, sizeof(zone));
        zone.struct_size = (uint32_t)sizeof(zone);
        expect(api->ocr_zone_scan_result_zone_at(NULL, 0, &zone) ==
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 null grouped OCR zone");
        expect(zone.region_count == 0,
               "grouped OCR zone initializes its count");

        memset(&ocr_region, 0xff, sizeof(ocr_region));
        ocr_region.struct_size = (uint32_t)sizeof(ocr_region);
        expect(api->ocr_zone_scan_result_region_at(
                   NULL, 0, 0, &ocr_region) ==
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 null grouped OCR region");
        expect(ocr_region.confidence == 0.0,
               "grouped OCR region initializes confidence");

        ocr_text = borrow("sentinel");
        expect(api->ocr_zone_scan_result_text_at(NULL, 0, 0, &ocr_text) ==
                   MADOPILOT_STATUS_INVALID_ARGUMENT,
               "ABI 1.4 null grouped OCR text");
        expect(ocr_text.data == NULL && ocr_text.len == 0,
               "grouped OCR text initializes its view");

        memset(&descriptor, 0xff, sizeof(descriptor));
        descriptor.struct_size = (uint32_t)sizeof(descriptor);
        expect(api->engine_ocr_descriptor(engine, &descriptor) ==
                   MADOPILOT_STATUS_UNSUPPORTED,
               "ABI 1.4 engine without OCR has no descriptor");
        expect(descriptor.backend_id.data == NULL &&
                   descriptor.backend_id.len == 0,
               "OCR descriptor initializes its borrowed views");
    }
...
int main(int argc, char** argv)
{
    ...
    printf("frozen header: abi %u.%u, table %u bytes as v1_4 declared it\n",
    ...
    if (failures != 0) {
        printf("madopilot-abi-compat-v1_4 failed %d check(s)\n", failures);
        return 1;
    }

    printf("madopilot-abi-compat-v1_4 complete\n");
    return 0;
}
