...
PRODUCT_SOURCE_PATHS = ("Cargo.toml", "Cargo.lock", "crates", "tools/setup-native.py",
                        "tools/native-release-profile")
...
def require_tracked_product_sources(command, git: str, checkout: Path, environment: dict[str, str]) -> None:
    # Ignore rules must not hide source inputs; only Python's generated bytecode is excluded.
    untracked = command("product-untracked-sources",
                        [git, "-C", str(checkout), "ls-files", "--others", "-z",
                         "--exclude=tools/native-release-profile/**/__pycache__/*.pyc",
                         "--", *PRODUCT_SOURCE_PATHS], environment)
    if untracked:
        raise ValueError("untracked product files cannot be reconstructed from the recorded revision and diff")
...
def prove(args: argparse.Namespace, checkout: Path, work: Path) -> int:
    ...
    try:
        ...
        require_tracked_product_sources(command, git, checkout, baseline)
        command("product-source-diff", [git, "-C", str(checkout), "diff", "--no-ext-diff", "--no-textconv", "--binary", "HEAD", "--",
                                        *PRODUCT_SOURCE_PATHS], baseline)
        ...
        refused_config = work / "tmp" / "non-regular-config"
        if sys.platform == "darwin":
            os.mkfifo(refused_config)
        else:
            refused_config.mkdir()
        try:
            command("consumer-config-refusal",
                    [str(binary), "--config", str(refused_config), "--allow-capture"],
                    environment, seconds=10, expected=1)
        finally:
            if sys.platform == "darwin":
                refused_config.unlink()
            else:
                refused_config.rmdir()
