...
class ExternalRustConsumerTests(unittest.TestCase):
    ...
    def test_untracked_product_files_cannot_hide_behind_gitignore(self):
        def git(*args):
            return subprocess.run(["git", "-C", str(self.checkout), *args], check=True,
                                  capture_output=True, text=True, timeout=10).stdout

        def command(_label, argv, _environment):
            return subprocess.run(argv, check=True, capture_output=True, text=True, timeout=10).stdout

        git("init", "--quiet")
        sources = self.checkout / "crates"
        sources.mkdir()
        source = sources / "new.rs"
        source.write_text("pub fn newly_used() {}\n", encoding="utf-8")
        with self.assertRaises(ValueError):
            consumer.require_tracked_product_sources(command, "git", self.checkout, {})
        git("add", "crates/new.rs")
        consumer.require_tracked_product_sources(command, "git", self.checkout, {})

        (self.checkout / ".git/info/exclude").write_text("crates/ignored.rs\n", encoding="utf-8")
        ignored = sources / "ignored.rs"
        ignored.write_text("pub fn also_used() {}\n", encoding="utf-8")
        with self.assertRaises(ValueError):
            consumer.require_tracked_product_sources(command, "git", self.checkout, {})
        git("add", "--force", "crates/ignored.rs")
        cache = self.checkout / "tools/native-release-profile/__pycache__"
        cache.mkdir(parents=True)
        (cache / "process_runner.cpython-313.pyc").write_bytes(b"generated bytecode")
        consumer.require_tracked_product_sources(command, "git", self.checkout, {})
